...
- GUI の自動アップデートは GitHub Releases の ZIP アセットをダウンロードし、一時 PowerShell スクリプトでアプリ終了後に配置先を上書きして再起動する。リリース ZIP はルート、または単一のペイロードフォルダ内に `AudioClient.GUI.exe` を含めること。
- GitHub Actions の release workflow は `vMAJOR.MINOR.PATCH` タグで動かし、GUI/CLI の自前成果物だけを ZIP 化する。Resonite/FrooxEngine/SkyFrost/Elements/Renderite/Awwdio などゲーム由来 DLL が混ざらないようにする。
