...
# CLAUDE.mdの更新
開発中にノウハウや留意点が見つかった場合は、該当するCLAUDE.mdに追記してください（全体共通なら本ファイル、CLI固有なら `AudioClient/CLAUDE.md`、GUI固有なら `AudioClient.GUI/CLAUDE.md`）。

# 調査・デバッグ知見
詳細な知見は `memory/` ディレクトリに蓄積しています。

- [FrooxEngine DynamicReferenceVariable 同期タイミング問題](memory/frooxengine_sync_timing.md) — リモートスロット到着直後はコンポーネントが未同期。`GlobalCoroutineManager.Post` の連打は sync より速すぎるため `Task.Delay(100ms)` で間隔を入れること
- [IStaticAssetProvider.URL でアセットURLを即時取得](memory/frooxengine_asset_url.md) — `provider.Asset?.AssetURL` はダウンロード完了後でないと null。`IStaticAssetProvider.URL` なら同期済み時点で取得可能
- [デバッグログ出力方法](memory/debugging_logging.md) — エンジン側は `UniLog.Log()`、GUI側は `Debug.WriteLine` がログに出ないためファイルログを使う
